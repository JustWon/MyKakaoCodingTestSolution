...
class LRUCache:
	...
	def set(self, key, value):
		...
		if self.counter > self.capacity:
			...
			self.counter -= 1
# 출처:[LeetCode OJ] LRU Cache - Hard

def Quiz2Solver(cacheSize, cities):

    if (not (0 <= cacheSize and cacheSize <=30)):
    ...
    for i, city in enumerate(cities):
        if (i < cacheSize): # initial caching
            lru.set(city, city)
            elapsed_time += cache_miss
            # print("%s cache init" % city)
        else:
            if(lru.get(city) != -1): # cache hit
                lru.set(city,city)
                elapsed_time += cache_hit
                # print("%s cache hit" % city)
            else:    # cache miss
                lru.set(city, city)
                elapsed_time += cache_miss
                # print("%s cache miss" % city)
